...
import CustomOrdersWorkspace from '../components/CustomOrdersWorkspace.jsx';

const NAV=[['overview','01','Overview'],['events','02','Events'],['spark','03','Spark Rush'],['poker','04','Poker tournaments'],['tickets','05','Tickets & orders'],['custom-orders','06','Custom orders'],['promotions','07','Promotions'],['audience','08','Audience'],['inbox','09','Inbox'],['content','10','Content'],['dating','11','Dating event'],['door','12','Door operations'],['analytics','13','Analytics'],['settings','14','Settings']];
const TITLES={overview:['Control room','What is happening across ISKRA right now.'],events:['Event operations','Build, publish and review every night.'],spark:['Spark Rush','Keep the arcade live before or between event announcements.'],poker:['Poker tournaments','Build, legally approve, run and audit free ticket tournaments.'],tickets:['Tickets & orders','Find a guest, payment or admission record.'],'custom-orders':['Custom orders','Build private, server-priced admission and VIP packages.'],promotions:['Promotions','Coordinate offers, codes and signup banners.'],audience:['Audience','Understand the people choosing to stay connected.'],inbox:['Inbox','Review and resolve incoming enquiries.'],content:['Content studio','Control public updates and event photography.'],dating:['Dating event','Open the social room and control King & Queen voting.'],door:['Door operations','Authorise devices without exposing the admin.'],analytics:['Analytics','Real visits, clicks and conversion signals.'],settings:['Settings','Workspace access and system boundaries.']};
...
export default function AdminPage(){
  ...
  const [sparkForm,setSparkForm]=useState({enabled:true,minParticipants:30});
  ...
  const load=async()=>{
    setBusy(true);
    try{
      const [next,nextOrders]=await Promise.all([api.adminSummary(),api.adminOrders(orderLog.pagination.page||1)]);
      setData(next);setSignedIn(true);
      setOrderLog(nextOrders);
      ...
      setSparkForm({enabled:next.sparkRush?.enabled!==false,minParticipants:next.sparkRush?.minParticipants||30});
  ...
  return <main className="admin-app">
    ...
    <div className="admin-workspace"><header className="admin-commandbar"><button className="admin-menu-button" onClick={()=>setMenuOpen(true)}>Menu</button><label className="admin-search"><span>⌕</span><input type="search" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search guests, orders, messages…"/></label><button className="admin-refresh" onClick={()=>load()} disabled={busy}>{busy?'Syncing…':'Sync data'}</button></header>
      <div className="admin-canvas"><header className="admin-pagehead"><div><p>ISKRA / {section.replaceAll('-',' ')}</p><h1>{currentTitle[0]}</h1><span>{currentTitle[1]}</span></div><div>{section==='events'&&<button className="btn btn-primary" onClick={()=>document.getElementById('event-editor')?.scrollIntoView({behavior:'smooth'})}>Create event</button>}{section==='door'&&<a className="btn btn-primary" href="/staff/scan">Open scanner</a>}</div></header>
        ...
        {section==='events'&&<EventsWorkspace data={data} form={eventForm} setForm={setEventForm} save={saveEvent} edit={event=>{setEventForm({...defaultEvent,...event,date:new Date(event.date).toISOString().slice(0,16),tags:(event.tags||[]).join(', '),badges:(event.badges||[]).join(', ')});requestAnimationFrame(()=>document.getElementById('event-editor')?.scrollIntoView({behavior:'smooth'}));}} remove={slug=>window.confirm('Remove this event from the schedule? This cannot be undone.')&&act(()=>api.adminDeleteEvent(slug),'Event removed.')}/>}
        {section==='spark'&&<SparkWorkspace data={data.sparkRush} form={sparkForm} setForm={setSparkForm} save={e=>{e.preventDefault();act(()=>api.adminSparkRush(sparkForm),`Spark Rush turned ${sparkForm.enabled?'on':'off'}.`);}}/>}
        {section==='poker'&&<PokerAdminWorkspace events={data.events||[]}/>}
        {section==='tickets'&&<TicketsWorkspace tickets={tickets} orders={orders} orderPagination={orderLog.pagination} changeOrderPage={changeOrderPage} busy={busy} resendOne={async order=>{if(!window.confirm(`Resend ${order.qty} ticket${order.qty===1?'':'s'} to ${order.email}?`))return false;return resend(()=>api.adminResendOrderTickets(order.id));}} resendSelected={async ids=>{if(!window.confirm(`Resend tickets for ${ids.length} selected order${ids.length===1?'':'s'}?`))return false;return resend(()=>api.adminResendTicketOrders(ids));}} resendAll={async()=>{if(!window.confirm('Resend tickets for every paid order with issued tickets? This will email every eligible buyer again.'))return false;return resend(()=>api.adminResendAllTicketOrders());}} update={(ticket,status)=>{const warning=status==='cancelled'?'Cancel this ticket? It will be rejected at the door.':status==='reserved'?'Restore this cancelled ticket?':null;if(warning&&!window.confirm(warning))return;act(()=>status==='redeemed'?api.adminRedeemTicket(ticket.id):api.adminTicketStatus(ticket.id,status),status==='redeemed'?'Guest checked in.':'Ticket updated.');}}/>}
        {section==='custom-orders'&&<CustomOrdersWorkspace events={data.events||[]}/>}
        ...
        {section==='inbox'&&<InboxWorkspace messages={messages} update={(id,status)=>act(()=>api.adminMessage(id,status),'Message updated.')} reply={(id,body,subject)=>act(()=>api.adminMessageReply(id,body,subject),'Reply sent.')}/>}
...
function EventsWorkspace({data,form,setForm,save,edit,remove}){
  const change=e=>setForm({...form,[e.target.name]:e.target.type==='checkbox'?e.target.checked:e.target.value});
  return <div className="admin-two-column admin-editor-layout">
    ...
    <form id="event-editor" className="admin-surface admin-form" onSubmit={save}>
      <div className="admin-section-title"><div><span>Event editor</span><h2>{form.slug?'Edit event':'Create event'}</h2></div>{form.slug&&<button type="button" className="admin-text-action" onClick={()=>setForm(defaultEvent)}>Clear</button>}</div>
      <p className="admin-form-note">Select Edit to load every event detail here. Date and time use the venue's Montréal schedule.</p>
      <div className="admin-field-grid">
        <label><span>Event title *</span><input name="title" value={form.title} onChange={change} required/></label>
        <label><span>URL slug</span><input name="slug" value={form.slug} onChange={change} placeholder="generated-from-title" readOnly={Boolean(form.slug)}/></label>
        <label><span>Date & time *</span><input type="datetime-local" name="date" value={form.date} onChange={change} required/></label>
        <label><span>Room</span><input name="room" value={form.room} onChange={change}/></label>
        <label className="full"><span>Support / subtitle</span><input name="support" value={form.support} onChange={change}/></label>
        <label className="full"><span>Venue address</span><input name="address" value={form.address} onChange={change}/></label>
        <label className="full"><span>Description</span><textarea name="description" rows="4" value={form.description} onChange={change}/></label>
        <label className="full"><span>Poster path or URL</span><input name="image" value={form.image} onChange={change}/></label>
        <label><span>Early bird price</span><input type="number" min="0" name="from" value={form.from} onChange={change}/></label>
        <label><span>Capacity</span><input type="number" min="0" name="capacity" value={form.capacity} onChange={change}/></label>
        <label><span>Spark reward minimum</span><input type="number" min="30" max="50" name="arcadeMinParticipants" value={form.arcadeMinParticipants} onChange={change}/></label>
        <label><span>Tags</span><input name="tags" value={form.tags} onChange={change}/></label>
        <label><span>Badges</span><input name="badges" value={form.badges} onChange={change}/></label>
      </div>
      <label className="admin-toggle"><input type="checkbox" name="active" checked={form.active} onChange={change}/><span><b>Publish to schedule</b><small>Publishing the next future event automatically attaches the open global Spark Rush board.</small></span></label>
      <button className="btn btn-primary">Save event changes</button>
    </form>
  </div>;
}

function SparkWorkspace({data,form,setForm,save}){
  const contest=data?.contest;
  return <form className="admin-surface admin-form admin-dating-control" onSubmit={save}>
    <div className="admin-section-title"><div><span>Global game control</span><h2>Spark Rush</h2></div><Status>{form.enabled?'open':'closed'}</Status></div>
    <p className="admin-body-copy">This switch controls Spark Rush independently from the schedule. If no future event exists, guests play on a live holding board. The board, players, attempts, and scores automatically move to the next eligible event when it is published.</p>
    <label className="admin-toggle"><input type="checkbox" checked={form.enabled} onChange={e=>setForm({...form,enabled:e.target.checked})}/><span><b>Spark Rush is open</b><small>Allow registration, daily attempts, score submission, and the live leaderboard.</small></span></label>
    <label><span>Minimum qualified players</span><input type="number" min="30" max="50" value={form.minParticipants} onChange={e=>setForm({...form,minParticipants:e.target.value})}/><small>Between 30 and 50 players before event rewards can be finalized.</small></label>
    <div className="admin-dating-warning"><b>{contest?.assigned?'Attached competition':'Pre-schedule holding board'}</b><p>{contest?`${contest.eventTitle} · ${contest.participantCount}/${contest.minParticipants} qualified players`:'The game is currently unavailable. Turn it on to open a board.'}</p></div>
    <button className="btn btn-primary">Save Spark Rush controls</button>
  </form>;
}

function TicketsWorkspace({tickets,orders,orderPagination,changeOrderPage,update,busy,resendOne,resendSelected,resendAll}){
  const [selected,setSelected]=useState([]);
  const eligible=orders.filter(order=>order.status==='paid'&&order.email);
  const toggle=id=>setSelected(current=>current.includes(id)?current.filter(value=>value!==id):[...current,id]);
  const allVisibleSelected=eligible.length>0&&eligible.every(order=>selected.includes(order.id));
  const toggleVisible=()=>setSelected(current=>allVisibleSelected?current.filter(id=>!eligible.some(order=>order.id===id)):[...new Set([...current,...eligible.map(order=>order.id)])]);
  return <><section className="admin-surface"><div className="admin-section-title admin-delivery-head"><div><span>Payment ledger</span><h2>Recent orders</h2><p>{orderPagination.total||0} total orders · choose recipients below</p></div><div className="admin-delivery-actions"><button onClick={()=>resendSelected(selected).then(sent=>sent&&setSelected([]))} disabled={busy||!selected.length}>Resend selected ({selected.length})</button><button className="danger" onClick={resendAll} disabled={busy}>Resend all tickets</button></div></div><OrderTable rows={orders} selected={selected} toggle={toggle} toggleVisible={toggleVisible} allVisibleSelected={allVisibleSelected} resendOne={resendOne} busy={busy}/><Pagination pagination={orderPagination} changePage={changeOrderPage} label="order"/></section><section className="admin-surface"><div className="admin-section-title"><div><span>Admission inventory</span><h2>Issued tickets</h2></div><p>{tickets.length} matching records</p></div><div className="admin-ticket-table">{tickets.map(t=><article key={t.id}><div><Status>{t.status}</Status><b>{t.reference}</b><span>{dateTime(t.createdAt)}</span></div><div><b>{t.buyerName}</b><span>{t.buyerEmail}</span></div><div><b>{t.eventTitle}</b><span>{t.tier} · {money(t.price)}</span></div><div className="admin-row-actions">{t.status==='reserved'&&<button onClick={()=>update(t,'redeemed')}>Check in</button>}{t.status!=='cancelled'&&<button onClick={()=>update(t,'cancelled')}>Cancel</button>}{t.status==='cancelled'&&<button onClick={()=>update(t,'reserved')}>Restore</button>}</div></article>)}{!tickets.length&&<Empty title="No ticket matches" copy="Try a guest name, email, reference or event title."/>}</div></section></>;
}
function OrderTable({rows,selected=[],toggle,toggleVisible,allVisibleSelected=false,resendOne,busy=false}){const selectable=Boolean(toggle);return <div className={`admin-order-table ${selectable?'with-actions':''}`}><div className="admin-table-head"><span>{selectable&&<input type="checkbox" checked={allVisibleSelected} onChange={toggleVisible} aria-label="Select all eligible orders on this page"/>} Customer</span><span>Event / tier</span><span>Payment</span><span>Delivery</span><span>Total</span><span>Created</span>{selectable&&<span>Action</span>}</div>{rows.map(row=>{const eligible=row.status==='paid'&&Boolean(row.email);return <article key={row.id}><div className="admin-order-customer">{selectable&&<input type="checkbox" checked={selected.includes(row.id)} onChange={()=>toggle(row.id)} disabled={!eligible||busy} aria-label={`Select tickets for ${row.buyerName||row.email||'guest'}`}/>}<span><b>{row.buyerName||'Guest'}</b><small>{row.email||'No email'}</small></span></div><div><b>{row.eventSlug}</b><small>{row.qty} × {row.tier}{row.code?` · ${row.code}`:''}</small></div><Status>{row.paymentStatus}</Status><div className="admin-order-delivery" title={row.emailLastResendError||''}><Status>{row.emailStatus||'pending'}</Status>{row.emailResendCount>0&&<small>Resent {row.emailResendCount}× · {dateTime(row.emailLastResentAt)}</small>}</div><b>{money(row.total)}</b><span>{dateTime(row.createdAt)}</span>{selectable&&<button className="admin-resend-one" onClick={()=>resendOne(row)} disabled={!eligible||busy}>Resend</button>}</article>})}{!rows.length&&<Empty title="No orders found" copy="Paid and pending checkout records will appear here."/>}</div>}
...
/* Reply composer. One send at a time: the button is disabled while a reply is
   in flight so a second click cannot email the guest twice. */
function ReplyComposer({message,onSend,onCancel}){
  const [body,setBody]=useState('');
  const [subject,setSubject]=useState(`Re: ${message.subject||'Your message to Project ISKRA'}`);
  ...
  const submit=async e=>{
    ...
    setSending(true);
    try{ await onSend(body.trim(),subject.trim()); }
    finally{ setSending(false); }
  };
  return <form className="admin-reply" onSubmit={submit}>
    <label><span>Subject</span>
      <input value={subject} maxLength={160} onChange={e=>setSubject(e.target.value)}/>
    </label>
    <label><span>Reply to {message.name} · {message.email}</span>
      <textarea value={body} rows={6} maxLength={limit} autoFocus
                placeholder="Write your reply…"
                onChange={e=>setBody(e.target.value)}/>
    </label>
    <div className="admin-reply-foot">
      <small>{body.length}/{limit}</small>
      <div>
...
function InboxWorkspace({messages,update,reply}){
  const [openId,setOpenId]=useState(null);
  return <section className="admin-inbox"><aside><span>Folders</span><button className="active">All messages <b>{messages.length}</b></button><button>Unread</button><button>Resolved</button><button>Archived</button></aside><div className="admin-surface"><div className="admin-section-title"><div><span>Contact requests</span><h2>Team inbox</h2></div></div><div className="admin-message-list">{messages.map(m=><article key={m._id} className={m.status==='new'?'unread':''}><header><div><Status>{m.status}</Status><b>{m.subject}</b></div><time>{dateTime(m.createdAt)}</time></header><h3>{m.name} <span>{m.email}</span></h3><p>{m.message}</p>{(m.category||m.eventDate||m.groupSize)&&<small>{m.category}{m.eventDate?` · ${new Date(m.eventDate).toLocaleDateString()}`:''}{m.groupSize?` · ${m.groupSize} guests`:''}</small>}
    {m.replies?.length>0&&<div className="admin-reply-thread">{m.replies.map((r,i)=><div key={i} className={r.deliveryStatus==='failed'?'is-failed':''}><span className="mono">{r.deliveryStatus==='failed'?'Not delivered':'Sent'} · {dateTime(r.sentAt)}{r.sentBy?` · ${r.sentBy}`:''}</span><p>{r.body}</p>{r.error&&<em>{r.error}</em>}</div>)}</div>}
    {openId===m._id&&<ReplyComposer message={m} onCancel={()=>setOpenId(null)} onSend={async(body,subject)=>{await reply(m._id,body,subject);setOpenId(null);}}/>}
    <footer>{openId!==m._id&&<button onClick={()=>setOpenId(m._id)}>Reply</button>}{m.status==='new'&&<button onClick={()=>update(m._id,'read')}>Mark read</button>}{m.status!=='archived'&&<button onClick={()=>update(m._id,'archived')}>Archive</button>}</footer></article>)}{!messages.length&&<Empty title="Inbox is clear" copy="No message matches the current search."/>}</div></div></section>;
}
