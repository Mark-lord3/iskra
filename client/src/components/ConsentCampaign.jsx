...
export default function ConsentCampaign(){
  ...
  return <>
    {!consent&&<aside className="consent-bar" aria-label={t('consent.label')}><div><b>{t('consent.title')}</b><p>{t('consent.copy')}</p></div><div><button onClick={()=>choose('essential')}>{t('consent.essential')}</button><button onClick={()=>choose('granted')}>{t('consent.allow')}</button></div></aside>}
    {campaign&&<aside className="campaign-popup" role="dialog" aria-modal="false" aria-labelledby="campaign-title"><button className="campaign-close" onClick={dismiss} aria-label={t('consent.dismiss')}>×</button><span>{t('consent.signal')}</span><h2 id="campaign-title">{campaign.title}</h2><p>{campaign.text}</p><a className="btn btn-primary" href={campaign.href} onClick={follow}>{campaign.cta} →</a><small>{t('consent.frequency')}</small></aside>}
  </>;
}
